...
struct chunk_header{
     ...
     */
    inline chunk_header(const dhyara::peer_address& target, const dhyara::peer_address& source, std::uint8_t length, std::uint8_t packet, std::uint8_t pending = 0): 
...
struct chunk{
    ...
    inline chunk(const dhyara::peer_address& target, const dhyara::peer_address& source, InputIt begin, std::uint8_t length, std::uint8_t packet, std::uint8_t pending = 0): _header(target, source, std::min(static_cast<std::uint8_t>(capacity), length), packet, pending) {
     ...
     * target of the chunk
     */
    inline dhyara::peer_address target() const { return _header.target(); }
    /**
     * source of the chunk
     */
    inline dhyara::peer_address source() const { return _header.source(); }
    /**
