...
struct data{
     ...
     */
    inline data(const dhyara::peer_address& target, const dhyara::peer_address& source): _target(target), _source(source), _packet(counter++) {}
    /**
     * Construct a data packet
     * \param target target address
     * \param packet packet id
     */
    inline data(const dhyara::peer_address& target, std::uint8_t packet): _target(target), _source(dhyara::peer_address::null()), _packet(packet) {}
    /**
     * Construct a data packet
     * \note an auto incremented number is used as packet id
     * \param target target address
     */
    inline data(const dhyara::peer_address& target): _target(target), _source(dhyara::peer_address::null()), _packet(counter++) {}
    /**
     * Construct a data packet
     * \param target target address
     * \param source source address
     * \param packet packet id
     */
    inline data(const dhyara::peer_address& target, const dhyara::peer_address& source, std::uint8_t packet): _target(target), _source(source), _packet(packet) {}
    /**
    ...
    inline data(const dhyara::peer_address& target, InputIt begin, InputIt end): _target(target), _source(dhyara::peer_address::null()), _packet(counter++) {
        fill(begin, end);
    }
    /**
