...
dhyara::utils::services::ping_impl::ping_impl(dhyara::network& network, std::uint8_t count, std::int8_t batch, std::uint8_t sleep): _network(network), _count(count), _batch(batch), _sleep(sleep), _first(0), _last(0), _wastage(0){
    using namespace std::placeholders;
    _conn_reply = _network.echo_reply().connect(std::bind(&dhyara::utils::services::ping_impl::reply, this, _1, _2));
}
